...
const root=new URL('../../out/reviews/house-cannonball/',import.meta.url),out=new URL('../../out/reviews/tape-stalls/',import.meta.url);
...
try{for(const name of ['rebound-reference-meteor-c1','rebound-porch-meteor-c1','rebound-bungalow-cannonball-c1','rebound-bungalow-cannonball-c2']){
 ...
 const camera={position:[-21,14,24],target:[0,3,0]},page=await browser.newPage({viewport:{width:960,height:640}});page.on('pageerror',e=>errors.push(String(e)));
 ...
 for(const time of name.includes('meteor')?[0,2.4,2.6,3,5,15]:[0,.2,.4,1,3,6]){
  ...
  writeFileSync(new URL(`${name}-${time}s.jpg`,out),Buffer.from(jpg,'base64'));
