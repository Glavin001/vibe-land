...
const sdk=process.env.PHYSX_DESTRUCTION_SDK??path.resolve(repo,'../physx-2');
...
await run('cargo',['build','--offline','--locked','--release','--manifest-path','native/Cargo.toml'],{CARGO_HOME:cargoHome,CUDA_HOME:'/usr/local/cuda-12.8',CARGO_TARGET_DIR:path.join(kit,'native/target')});
for(const mode of modes){
 ...
 const hardware=execFileSync('nvidia-smi',['--query-gpu=name,driver_version,memory.used,utilization.gpu','--format=csv,noheader'],{encoding:'utf8'}).trim();
