...
const browser=await chromium.launch({headless:true,ignoreDefaultArgs:['--disable-dev-shm-usage'],args:['--no-sandbox','--disable-gpu-sandbox','--disable-gpu-shader-disk-cache','--use-gl=angle',...(renderingBackend==='software'?['--use-angle=swiftshader','--enable-unsafe-swiftshader']:process.platform==='darwin'?['--use-angle=metal','--ignore-gpu-blocklist']:['--use-angle=vulkan','--enable-features=Vulkan','--ignore-gpu-blocklist'])]});
try{
 ...
 await page.goto(`${origin}/?asset=${encodeURIComponent(asset)}&clean`,{waitUntil:'domcontentloaded',timeout:60000});
  ...
  const startupError=await page.evaluate(()=>window.__TOWN_KIT__?.error);if(startupError)throw Error(startupError);
 ...
 if(mode&&['prop','tree'].includes(stats.kind)){await page.evaluate(()=>window.__TOWN_KIT__.frameFragments());captureCameras=await page.evaluate(()=>window.__TOWN_KIT__.damageCameras());}
 ...
 await writeFile(path.join(out,'contact-sheet.html'),html);execFileSync(process.env.TOWN_KIT_PYTHON??'python3',[path.join(kit,'scripts/contact-sheet.py'),out,JSON.stringify(captures),`Bayline / ${asset} / ${mode||'intact'} / ${stats.hash}`]);
 await writeFile(path.join(out,'visual-report.json'),JSON.stringify({asset,mode,imageFormat,renderingBackend,finish:stats.finish,finishTuning:stats.finishTuning,hash:stats.hash,metadataSha256:sha(await readFile(path.join(kit,'out',`${asset}.meta.json`))),recordingSha256:mode?sha(await readArtifact(path.join(out,'recording.json'))):null,provenance:await sourceProvenance(),captures,cameraStrategy:mode&&['prop','tree'].includes(stats.kind)?'settled-fragment-bounds':'saved-cameras',cameras:captureCameras,errors,nativeStatus,viewport:[1600,1000]},null,2));console.log(out);
