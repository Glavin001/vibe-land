...
const key='bayline-framed-36',input=readFileSync(`${KIT}/out/bayline-proven-36-fixedground.vlsp`);
...
for(let i=0;i<h.instances.length;i++){
 ...
 if(lot&&['house','bungalow'].includes(lot.builder)){
  const cacheKey=JSON.stringify([lot.builder,lot.options]);
  if(!cache.has(cacheKey)){
   const a=(lot.builder==='house'?buildFramedPorchHouse:buildFramedBungalow)(lot.options),validation=validate(a.pack);
...
metadata.acceptance={readyForRelease:false,note:'Experimental framed town. Selected isolated skeletons and intact houses passed; full variant, assembled idle, traversal and damaged-settling qualification remains incomplete. Some rubble cases fail. No old stress warm values are reused.'};
...
const args={key,title:'Bayline · Framed homes and high street',metadata,provenance};
