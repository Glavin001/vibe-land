 ...
 */
export function createFramedEnvelope(b,{floorBounds,singleSkin=false}) {
 const plain=createEnvelope(b,{floorBounds}),B=plain.B;
 function wall(axis,at,u0,u1,y0,y1,openings=[],material=M.wall,role='wall',frameOnly=false) {
 ...
 function facade(axis,at,u0,u1,y0,y1,openings){
  // Single-skin timber boarding carries no roof load; the exposed inner face
  // is the same board, rather than a second solid infill collider.
  const frameStart=b.s.nodes.length;
  wall(axis,at,u0,u1,y0,y1,openings,M.wall,'wall',singleSkin);
  // Board butt joints lie over studs, so each independent board has real
  // attachment area. Equal subdivisions can leave short boards between studs.
  const along=axis==='z'?'x':'z';
  const boardJoints=singleSkin?[...new Set(b.s.nodes.slice(frameStart).flatMap((n,i)=>b.s.nodeTypes[frameStart+i]==='frame-post'?[n.centroid[along]]:[]))].sort((a,c)=>a-c):[];
  const outward=at[0]<0?-1:1,face=outward<0?at[0]:at[1],skin=outward<0?[face-.03,face]:[face,face+.03];
  const course=singleSkin?.76:.19;
  for(let y=y0;y<y1-.001;y+=course){
   const top=Math.min(y+(singleSkin?course:.176),y1),ys=[...new Set([y,top,...openings.flatMap(o=>o.slice(2,4)).filter(v=>v>y&&v<top)])].sort((a,c)=>a-c);
   for(let j=0;j<ys.length-1;j++){
    ...
    for(const [a,c]of spans){
     const cuts=[a];
     if(singleSkin){
      while(c-cuts.at(-1)>2.4+1e-5){
       const candidates=boardJoints.filter(u=>u>cuts.at(-1)+1e-5&&u<c-1e-5);
       const next=candidates.filter(u=>u<=cuts.at(-1)+2.4+1e-5).at(-1)??candidates[0];
       if(next===undefined)break;cuts.push(next);
      }
     }
     cuts.push(c);
     for(let k=0;k<cuts.length-1;k++){
      const left=cuts[k],right=cuts[k+1],min=axis==='z'?[left,lo,skin[0]]:[skin[0],lo,left],max=axis==='z'?[right,hi,skin[1]]:[skin[1],hi,right];
      const split=singleSkin?[1,1,1]:axis==='z'?[Math.ceil((right-left)/2.4),1,1]:[1,1,Math.ceil((right-left)/2.4)];
      B(min,max,M.siding,'siding',split);
     }
    }
   }
  }
...
/** Assign structural connections independently of the cosmetic attachments. */
export function frameBondPolicy(pack,{impactProfile}={}){
 const s=pack.scenario;
 // Adjacent boards are nailed to studs, not welded to neighboring boards.
 // Subdivisions within one authored board keep their ordinary breakable bonds.
 if(impactProfile==='residential-v2')s.bonds=s.bonds.filter(b=>!(s.nodeTypes[b.node0]==='siding'&&s.nodeTypes[b.node1]==='siding'&&s.nodePieces[b.node0]!==s.nodePieces[b.node1]));
 // Preserve the original profile. The residential candidate uses timber-joint
 // limits for structural connections, including authored member subdivisions.
 const frame=new Set(['frame-post','frame-beam','floor','ceiling','roof-rafter','roof-ridge','roof-post','stair','ceiling-joist']);
 for(const bond of s.bonds){
  const a=s.nodeTypes[bond.node0],c=s.nodeTypes[bond.node1];
  if(frame.has(a)&&frame.has(c))bond.m=['residential-v1','residential-v2'].includes(impactProfile)?M.joint:M.frame;
  if(['residential-v1','residential-v2'].includes(impactProfile)&&[a,c].some(t=>['roof','porch-roof','ceiling-panel'].includes(t)))bond.m=M.fastener;
