...
export class GrassInteraction {
  ...
  begin(time: number, cameraX: number, cameraZ: number): boolean {
    ...
    for (let i = 0; i < this.pressure.length; i++) {
      ...
      if (held < dt) {
        ...
        this.dx[i] *= falloff; this.dz[i] *= falloff;
        if (this.pressure[i] < 0.003) this.pressure[i] = this.dx[i] = this.dz[i] = 0;
        this.dirty = true;
      }
    }
    return true;
  ...
  private scroll(x: number, z: number): void {
    ...
    if (!sx && !sz) return;
    for (const channel of [this.pressure, this.dx, this.dz, this.hold]) {
  ...
  stamp(contact: GrassContact): boolean {
    ...
    if (![x, z, contact.radiusX, contact.radiusZ, contact.yaw ?? 0, contact.pressure ?? 1,
      contact.hold ?? 0.2, contact.fromX ?? x, contact.fromZ ?? z].every(Number.isFinite)) return false;
  ...
  private ellipse(x: number, z: number, rx: number, rz: number, yaw: number, pressure: number, hold: number, box: boolean): void {
    const reach = Math.max(rx, rz) + this.texel;
    ...
    for (let iz = minZ; iz <= maxZ; iz++) for (let ix = minX; ix <= maxX; ix++) {
      ...
      if (weight >= this.pressure[i]) {
        ...
        this.dx[i] = dx / length * weight;
        this.dz[i] = dz / length * weight;
        this.hold[i] = hold;
        this.dirty = true;
      } else if (weight > 0.5) {
  ...
  clear(): void {
    this.pressure.fill(0); this.dx.fill(0); this.dz.fill(0); this.hold.fill(0);
