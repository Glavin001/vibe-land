...
export const EXECUTE_JS_DESCRIPTION =
...
/**
 * Core executor for the `execute_js` tool. Shared between the AI SDK tool
 * wrapper (cloud providers) and the local Qwen3 tool loop.
 */
export async function runExecuteJs(
...
export function createExecuteJsTool(accessors: WorldAccessors) {
  return tool({
    description: EXECUTE_JS_DESCRIPTION,
    inputSchema,
    execute: (input) => runExecuteJs(input, accessors),
  });
}
...
export const ROLLBACK_DESCRIPTION =
...
export async function runRollback(
  input: RollbackInput,
  accessors: WorldAccessors,
): Promise<{ ok: boolean; message: string; commitId?: string }> {
...
export function createRollbackTool(accessors: WorldAccessors) {
  return tool({
    ...
    execute: (input) => runRollback(input, accessors),
  });
}
