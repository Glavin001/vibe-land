...
export function AudioMixControls({ showPresets = true, onPreset }: { showPresets?: boolean; onPreset?: (preset: MixPreset) => void }) {
  ...
  const slider = (key: 'master' | 'impact' | 'detail' | 'bass' | 'space' | 'flyby' | 'ringing', label: string, hint?: string) => (
    <label className="sound-slider" key={key}>
      ...
      <input aria-label={label} type="range" min="0" max={key === 'master' || key === 'ringing' ? 1 : 1.5} step="0.01" value={settings[key]} disabled={key === 'space' && settings.acoustics === 'dry'} onChange={e => setAudioSettings({ [key]: Number(e.target.value) } as Partial<AudioSettings>)} />
  ...
  return <div className="sound-controls">
    ...
    <div className="sound-select-row"><label>Room sound<select aria-label="Reflections mode" value={settings.acoustics} onChange={e => setAudioSettings({ acoustics: e.target.value === 'reflections' ? 'reflections' : 'dry' })}><option value="dry">Dry · direct sound</option><option value="reflections">With reflections</option></select></label></div>
