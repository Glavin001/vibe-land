...
impl CityRuntime {
    ...
    pub fn physx(sim_hz: u32, world: &mut World) -> anyhow::Result<Self> {
        let (_, manifest, _) = manifest_asset()
            .context("city scene asset unavailable (destruction/assets/scenes)")?;
        let manifest = manifest.clone();
        let settings = city_stress_settings();
...
/// directly -- the trace recorder in `bin/record_city_trace.rs` -- gets the
/// same tuning the match server uses instead of a second copy that drifts.
#[cfg(feature = "destruction")]
pub fn city_stress_settings() -> StressSolverSettings {
