...
use crate::settle::{SettleConfig, SettleTracker};

// The collision groups, error type and manifest conversions every PhysX-backed
// backend shares; defined once so the Blast and native paths install identical
// assets.
pub use crate::bridge_authoring::{
    CityDestructionError, CHUNK_COLLISION_MASK, GROUP_BATTERY, GROUP_CHUNK, GROUP_DYNAMIC,
    GROUP_PLAYER, GROUP_STATIC, GROUP_VEHICLE,
};
pub(crate) use crate::bridge_authoring::{authored_structure, ffi_settings};
...
impl CityDestruction {
    pub fn build(
        ...
        let ffi_settings = ffi_settings(&settings);
