...
for case,expected in summary['cases'].items():
 ...
 assert sidecar['manifestHash']==load('expected-live-manifest.json')['city_manifest_hash']
 assert tape['metadata']['manifest_hash']==sidecar['manifestHash']
...
assert summary['awake_target_5000_reached']==True
assert summary['matches_live_city_manifest']==True
...
assert live['city_manifest_hash']==record['metadata']['manifest_hash']
...
print('PASS: 200 identical external inputs, same-tick/miss coverage, negative metadata cases, restored city; exact live manifest verified; physical divergence retained; no performance claim')
