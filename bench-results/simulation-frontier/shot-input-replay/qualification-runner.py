...
base=Path('/tmp/city-shot-tape-live-manifest')
base.mkdir(exist_ok=False)
config=json.loads(Path('/tmp/profile-city-restore-config.json').read_text())
expected_live=json.loads(Path('/tmp/shot-tape-expected-live-manifest.json').read_text())
(base/'expected-live-manifest.json').write_text(json.dumps(expected_live,indent=2)+'\n')
...
provenance={'binary_sha256':sha(binary),'game_revision':revision(config['game_root']),
 ...
 'sources':{rel:sha(Path(config['game_root'])/rel) for rel in ['server/src/bin/record_city_trace.rs','server/src/bin/record_city_trace/shot_tape.rs','destruction/src/city_config.rs','server/src/city.rs']}}
...
def run(name,seconds,flags,extra_env=None):
 ...
 assert sidecar['chunks']==96420
 assert sidecar['manifestHash']==expected_live['city_manifest_hash'],sidecar['manifestHash']
 stats={'chunks':sidecar['chunks'],'manifest_hash':sidecar['manifestHash'],'ticks':len(rows),'attempted':sidecar['shotInputs'],'hits':sidecar['shotHits'],
...
negative=[]
for label,flags in [('weapon',['--seconds','20']),('duration',['--seconds','2']),('scene',['--seconds','20'])]:
 out=base/('negative-'+label);out.mkdir()
 target=out/'must-not-exist.trace'
 input_tape = Path('/tmp/city-shot-tape-qualification/record/shots.json') if label=='scene' else record/'shots.json'
 command=[str(binary),'--scene',config['scene'],'--grid','2','--shot-tape-in',str(input_tape),
...
a=list(csv.DictReader((record/'metrics.csv').open()));b=list(csv.DictReader((replay/'metrics.csv').open()))
first=None
for x,y in zip(a,b):
 keys=[k for k in ['bodies','awake','bonds'] if x[k]!=y[k]]
 if keys:
  first={'tick':int(x['tick']),'record':{k:x[k] for k in keys},'replay':{k:y[k] for k in keys}}
  break
summary={'first_counter_divergence':first,'matches_live_city_manifest':True,'full_multiplayer_performance_gate':False,
 'awake_target_5000_reached':max(results[c]['max_awake'] for c in ['record','replay'])>=5000,'scope':'Exact external input replay and error handling; not deterministic physics or a performance comparison',
