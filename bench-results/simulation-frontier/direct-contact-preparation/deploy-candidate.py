...
with (deploy.STATE/'lock').open('w') as lock:
 ...
 oldenv=dict(d['env'])
 backup=deploy.STATE/'web-fps-server-before-contact-publication'
 shutil.copyfile(f"/proc/{d['server']}/exe",backup);backup.chmod(0o700)
 (deploy.STATE/'before-contact-publication.json').write_text(json.dumps(oldenv))
 ...
 active=deploy.STATE/f'web-fps-server-contact-publication-{time.time_ns()}'
 ...
 (root/'bench-results/simulation-frontier/direct-contact-preparation/deployment.json').write_text(json.dumps(report,indent=2)+'\n')
