...
label=sys.argv[1]
trace=Path(sys.argv[2]) if len(sys.argv)>2 else root/'target/gpu-activity/release/record-city-trace'
...
cmd=[str(trace),'--scene','destruction/assets/scenes/fractured-downtown.json','--grid','2','--seconds','10','--shots','100','--shot-interval-ticks','4','--targets','27','--output','/dev/null','--metrics-out',str(out/(label+'.csv')),'--timings-out',str(out/(label+'.jsonl'))]
