...
decision=json.loads((root/'bench-results/simulation-frontier/gpu-contact-order/qualification-decision.json').read_text())
assert decision['eligible_for_rollout'], 'GPU contact ordering rollout held by qualification: '+decision['reason']
...
with (deploy.STATE/'lock').open('w') as lock:
 ...
 oldenv=dict(d['env'])
 backup=deploy.STATE/'web-fps-server-before-gpu-contact-order'
 shutil.copyfile(f"/proc/{d['server']}/exe",backup);backup.chmod(0o700)
 (deploy.STATE/'before-gpu-contact-order.json').write_text(json.dumps(oldenv))
 candidate_env=dict(oldenv,PHYSX_ROOT=str(sdk),VIBE_PHYSX_DIRECT_GPU=mode,VIBE_PHYSX_GPU_CONTACT_ORDER='1')
 candidate_env['LD_LIBRARY_PATH']=str(sdk/'bin/linux.x86_64/release')+':/usr/local/cuda/lib64:'+oldenv.get('LD_LIBRARY_PATH','')
 active=deploy.STATE/f'web-fps-server-gpu-contact-order-{time.time_ns()}'
 ...
 try:
  ...
  assert actual['env']['VIBE_PHYSX_GPU_CONTACT_ORDER']=='1'
  assert actual['env'].get('VIBE_PHYSX_GPU_CONTACT_ORDER_VERIFY','0')=='0'
 ...
 report=dict(binary_sha256=expected,previous_binary_sha256=digest(backup),sdk_manifest_sha256=digest(sdk/'gpu-activity-manifest.json'),direct_gpu=mode,gpu_contact_order='1',verification=result)
 (root/'bench-results/simulation-frontier/gpu-contact-order/deployment.json').write_text(json.dumps(report,indent=2)+'\n')
