...
out=root/'bench-results/simulation-frontier/contact-wrench/qualification'
...
commands={
 ...
 'collapse':[str(trace),'--scene','destruction/assets/scenes/fractured-downtown.json','--grid','1','--seconds','60','--shots','12','--shot-interval-ticks','10','--targets','1','--aim-lock','--output','/dev/null','--metrics-out',str(out/(label+'.csv')),'--summary-out',str(out/(label+'-summary.json'))],
 'audit':[str(trace),'--scene','destruction/assets/scenes/fractured-downtown.json','--grid','2','--seconds','15','--shots','100','--shot-interval-ticks','4','--targets','27','--output','/dev/null','--metrics-out',str(out/(label+'.csv')),'--timings-out',str(out/(label+'.jsonl'))]
}
...
env['VIBE_PHYSX_GPU_CONTACT_ORDER']=sys.argv[5]
assert env['VIBE_PHYSX_GPU_CONTACT_ORDER'] in ('0','1')
if mode=='audit':
 env['VIBE_PHYSX_GPU_CONTACT_ORDER_VERIFY']='1'
 # Hoist verification disables the aggregate path; this audit must exercise it.
 env['VIBE_PHYSX_BONDLESS_HOIST_VERIFY']='0'
if mode=='audit':
 for key in ('BLAST_BOND_STRESS_GPU_VERIFY','BLAST_BOND_STRESS_COMPACT_VERIFY','BLAST_FRACTURE_CANDIDATES_VERIFY','BLAST_RESIM_RESTORE_VERIFY','VIBE_PHYSX_DRAIN_PARALLEL_VERIFY','VIBE_CITY_BOND_SAMPLE_VERIFY'):env[key]='1'
