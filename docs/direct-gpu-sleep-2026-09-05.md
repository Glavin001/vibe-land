...
## Implemented prototype and production gates

The narrow engine extension is now implemented and measured; see [the implementation report](gpu-activity-prototype-2026-09-05.md) for code, correctness results and matched timings. It decouples bulk state readback from activity transitions. The GPU already computes sleep candidates; retain poses, velocities, contact wrenches and stress inputs on device, and export compact activity transitions to the existing CPU island manager where necessary. Read current motion only for transitions or CPU consumers that actually require it. This preserves more existing wake/contact behavior while testing the value of avoiding bulk transfers. Its design must cover commands on sleeping bodies and metadata updates that could otherwise upload stale CPU poses. The prototype is opt-in; the remaining full-city acceptance requirements below still apply before production migration.
