...
`fractured-town.json` is a 49,070-chunk low-rise town — 63 buildings, nothing
over four storeys — mixing the city exporter's Voronoi-fractured archetypes
with the authored houses and parking garages, set into the same 12 m streets.
Generated by `scripts/export-fractured-town.mjs` in blast-stress-solver-2,
which merges the two material tables (six fractured + fourteen authored,
authored indices offset by six):

```bash
cd /root/workspace/blast-stress-solver-2/blast/blast-stress-solver
OUTPUT=/root/workspace/vibe-land-4/destruction/assets/scenes/fractured-town.json \
  node scripts/export-fractured-town.mjs
```

Serve it with `VIBE_CITY_SCENE=fractured-town.json`. It stands on the native
backend (0 bonds broken over 45 s idle, tick 4.5 ms avg on the 4090), and
the manifest is 25 MB against downtown's 7.5.
