#!/usr/bin/env bash
#
# Clone (or verify) the PhysX repo that hosts the `blast-stress-solver`
# Rust crate, at the exact SHA pinned in scripts/blast-pinned-sha.txt,
# into third_party/PhysX.
#
# The target directory is gitignored so it never lands in the vibe-land
# history.  It must exist before any `cargo` invocation because
# Cargo.toml contains:
#
#   [patch.crates-io]
#   blast-stress-solver = { path = "third_party/PhysX/blast/blast-stress-solver-rs" }
#
...
BRANCH="feat/rapier-destruction"
...
if [[ -d "${VENDOR_DIR}/.git" ]]; then
  ...
  if [[ "${CURRENT_SHA}" == "${PINNED_SHA}" ]]; then
    ...
    git -C "${VENDOR_DIR}" reset --hard "${PINNED_SHA}" 2>/dev/null \
...
# Apply all blast-*.patch files from patches/ idempotently.
